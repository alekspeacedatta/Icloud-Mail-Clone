    ...
    <>
      {filteredEmails.map((item: Email) => (
        <>
          <div
            key={item.id}
            className={`px-6 py-2 rounded-xl cursor-pointer border-b rounded-b-none border-[#35353a] leading-none ${SelectedMessageId === item.id ? `bg-[#009aff] rounded-b-xl` : `hover:bg-[#35353a]`}`}
            onClick={() => {
              setSelectedMessageId(item.id)
            }}
          >
            <div className="flex gap-1 justify-between">
              <h4 className="text-[17px] text-zinc-100 font-semibold truncate">
                {item.Author}
              </h4>
              <p className="text-sm text-[#ffffff80] font-medium">14:34</p>
            </div>
            <h5 className="text-zinc-100 mt-1 text-sm truncate">
              {item.subject}
            </h5>
